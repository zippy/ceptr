...
char *_f_if(int *fp,Flow *f,T *r) {
	...
	if (val == TRUE_VALUE) {
	    ...
	    T *dummy = _f_new(r); // make a dummy node for the true value
	    f->phase = 3;
	}
...
char *_f_def(int *fp,Flow *f,T *r) {
	...
	int def_id = next_id();
	T *d = _d_sys_def(label,def_id,0,0);
