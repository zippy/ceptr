...
void base_defs() {
  ...
  sP(SYS_CONTEXT,LTE_INT,"test less than or equal",0);
  sP(SYS_CONTEXT,GTE_INT,"test greater than or equal",0);
  sP(SYS_CONTEXT,CONCAT_STR,"concatinate strings",0);
  sP(SYS_CONTEXT,RESPOND,"respond to signal",0);
  sP(SYS_CONTEXT,SEND,"send a signal",0);
  sP(SYS_CONTEXT,EXPECT_ACT,"block run tree until expectation met and then act",0);
  sP(SYS_CONTEXT,QUOTE,"quote a process so that it can be passed as a value",0);
  sP(SYS_CONTEXT,INTERPOLATE_FROM_MATCH,"interploate match result in place",0);
  sP(SYS_CONTEXT,RAISE,"raise reduction error",0);
  sP(SYS_CONTEXT,STREAM_READ,"read from a stream",0);
  sP(SYS_CONTEXT,STREAM_WRITE,"write to a stream",0);
  sP(SYS_CONTEXT,STREAM_AVAILABLE,"check a stream for data availability",0);
  sP(SYS_CONTEXT,REPLICATE,"replicate process",0);
  sP(SYS_CONTEXT,LISTEN,"plant a listener",0);
