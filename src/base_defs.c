...
void base_defs() {
  ...
  sY(SYS_CONTEXT,SIGNATURE_PROCESS,STRUCTURE);
  ...
  sP(SYS_CONTEXT,NOOP,"no-op","result",TREE,0);
  sP(SYS_CONTEXT,IF,"if","result",TREE,"condition",SIGNATURE_PROCESS,BOOLEAN,"then",SIGNATURE_STRUCTURE,TREE,"else",SIGNATURE_STRUCTURE,TREE,0);
  sP(SYS_CONTEXT,ADD_INT,"addition","sum",INTEGER,"augend",SIGNATURE_STRUCTURE,INTEGER,"addend",SIGNATURE_STRUCTURE,INTEGER,0);
  sP(SYS_CONTEXT,SUB_INT,"subtract","difference",INTEGER,"minuend",SIGNATURE_STRUCTURE,INTEGER,"subtrahend",SIGNATURE_STRUCTURE,INTEGER,0);
  sP(SYS_CONTEXT,MULT_INT,"multiply","product",INTEGER,"multiplicand",SIGNATURE_STRUCTURE,INTEGER,"multiplier",SIGNATURE_STRUCTURE,INTEGER,0);
  sP(SYS_CONTEXT,DIV_INT,"divide","quotient",INTEGER,"dividend",SIGNATURE_STRUCTURE,INTEGER,"divisor",SIGNATURE_STRUCTURE,INTEGER,0);
  sP(SYS_CONTEXT,MOD_INT,"modulo","remainder",INTEGER,"dividend",SIGNATURE_STRUCTURE,INTEGER,"divisor",SIGNATURE_STRUCTURE,INTEGER,0);
  sP(SYS_CONTEXT,EQ_INT,"test equality","result",TREE,0);
  sP(SYS_CONTEXT,LT_INT,"test less than","result",TREE,0);
  sP(SYS_CONTEXT,GT_INT,"test greater than","result",TREE,0);
  sP(SYS_CONTEXT,LTE_INT,"test less than or equal","result",TREE,0);
  sP(SYS_CONTEXT,GTE_INT,"test greater than or equal","result",TREE,0);
  sP(SYS_CONTEXT,CONCAT_STR,"concatinate strings","result",TREE,0);
  sP(SYS_CONTEXT,RESPOND,"respond to signal","result",TREE,0);
  sP(SYS_CONTEXT,SEND,"send a signal","response",TREE,0);
  sP(SYS_CONTEXT,EXPECT_ACT,"block run tree until expectation met and then act","result",TREE,0);
  sP(SYS_CONTEXT,QUOTE,"quote a process so that it can be passed as a value","result",TREE,0);
  sP(SYS_CONTEXT,INTERPOLATE_FROM_MATCH,"interploate match result in place","result",TREE,0);
  sP(SYS_CONTEXT,RAISE,"raise reduction error","result",TREE,0);
  sP(SYS_CONTEXT,STREAM_READ,"read from a stream","result",TREE,0);
  sP(SYS_CONTEXT,STREAM_WRITE,"write to a stream","result",TREE,0);
  sP(SYS_CONTEXT,STREAM_AVAILABLE,"check a stream for data availability","result",TREE,0);
  sP(SYS_CONTEXT,REPLICATE,"replicate process","result",TREE,0);
  sP(SYS_CONTEXT,LISTEN,"plant a listener","result",TREE,0);
