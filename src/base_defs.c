...
void base_defs(SemTable *sem) {
  ...
  sP(SYS_CONTEXT,REQUEST,"send a request to a receptor","response",SIGNATURE_ANY,NULL_STRUCTURE,"of",SIGNATURE_SYMBOL,RECEPTOR_ADDRESS,"on",SIGNATURE_SYMBOL,ASPECT_IDENT,"carrier",SIGNATURE_SYMBOL,CARRIER,"message",SIGNATURE_ANY,NULL_STRUCTURE,"expect response on",SIGNATURE_SYMBOL,RESPONSE_CARRIER,"until",SIGNATURE_OPTIONAL,SIGNATURE_SYMBOL,END_CONDITIONS,"callback",SIGNATURE_OPTIONAL,SIGNATURE_PROCESS,NULL_SYMBOL,0);
  ...
  sP(SYS_CONTEXT,RESPOND,"respond to signal","response id",SIGNATURE_SYMBOL,SIGNAL_UUID,"respond on",SIGNATURE_SYMBOL,CARRIER,"response contents",SIGNATURE_ANY,NULL_STRUCTURE,0);
  ...
  sP(SYS_CONTEXT,INTERPOLATE_FROM_MATCH,"interploate match result in place","result",SIGNATURE_ANY,NULL_STRUCTURE,0);
  ...
  sY(SYS_CONTEXT,SERVER,RECEPTOR_ADDRESS);
  sY(SYS_CONTEXT,CLIENT,RECEPTOR_ADDRESS);
  sY(SYS_CONTEXT,PING,NULL_STRUCTURE);
  sY(SYS_CONTEXT,YUP,NULL_STRUCTURE);
  sY(SYS_CONTEXT,HANDLER,PROCESS);
  sData(SYS_CONTEXT,pattern1,PATTERN,SEMTREX_SYMBOL_LITERAL,SEMTREX_SYMBOL,PING);
  sData(SYS_CONTEXT,action1,ACTION,NOOP);
  sData(SYS_CONTEXT,pattern2,PATTERN,SEMTREX_SYMBOL_LITERAL,SEMTREX_SYMBOL,YUP);
  sData(SYS_CONTEXT,action2,GOAL,HANDLER);
  sProt(SYS_CONTEXT,ALIVE,ROLE,SERVER,ROLE,CLIENT,GOAL,HANDLER,INTERACTION,"alive",EXPECT,SERVER,CLIENT,pattern1,action1,EXPECT,CLIENT,SERVER,pattern2,action2);
