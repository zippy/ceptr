...
void testDefSysDefs() {
    ...
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,SYS_CONTEXT)->definitions,SEM_TYPE_STRUCTURE),STRUCTURE_DEF.id)),"(STRUCTURE_DEFINITION (STRUCTURE_LABEL:STRUCTURE_DEF) (STRUCTURE_SYMBOL_SET (STRUCTURE_SYMBOL:STRUCTURE_SYMBOL) (STRUCTURE_SYMBOL:STRUCTURE_SEQUENCE) (STRUCTURE_SYMBOL:STRUCTURE_SYMBOL_SET) (STRUCTURE_SYMBOL:STRUCTURE_OR) (STRUCTURE_SYMBOL:STRUCTURE_ZERO_OR_MORE) (STRUCTURE_SYMBOL:STRUCTURE_ONE_OR_MORE) (STRUCTURE_SYMBOL:STRUCTURE_ZERO_OR_ONE) (STRUCTURE_SYMBOL:SYMBOL_OF_STRUCTURE) (STRUCTURE_SYMBOL:STRUCTURE_ANYTHING)))");

    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,SYS_CONTEXT)->definitions,SEM_TYPE_STRUCTURE),CSTRING.id)),"(STRUCTURE_DEFINITION (STRUCTURE_LABEL:CSTRING) (STRUCTURE_SYMBOL:NULL_SYMBOL))");

    // an example of a defined process (IF)
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,SYS_CONTEXT)->definitions,SEM_TYPE_PROCESS),IF.id)),"(PROCESS_DEFINITION (PROCESS_NAME:IF) (PROCESS_INTENTION:if) (process:NULL_PROCESS) (PROCESS_SIGNATURE (OUTPUT_SIGNATURE (SIGNATURE_LABEL:result) (SIGNATURE_PASSTHRU)) (INPUT_SIGNATURE (SIGNATURE_LABEL:condition) (SIGNATURE_PROCESS:BOOLEAN)) (INPUT_SIGNATURE (SIGNATURE_LABEL:then) (SIGNATURE_ANY)) (INPUT_SIGNATURE (SIGNATURE_LABEL:else) (SIGNATURE_ANY) (SIGNATURE_OPTIONAL))))");

    // an example of a process with code
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,DEV_COMPOSITORY_CONTEXT)->definitions,SEM_TYPE_PROCESS),respond_with_yup.id)),"(PROCESS_DEFINITION (PROCESS_NAME:respond_with_yup) (PROCESS_INTENTION:respond with yup) (process:RESPOND (SIGNAL_REF:/1/4) (YUP)) (PROCESS_SIGNATURE (OUTPUT_SIGNATURE (SIGNATURE_LABEL:response id) (SIGNATURE_SYMBOL:SIGNAL_UUID))))");

    // an example of a defined protocol (alive)
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,DEV_COMPOSITORY_CONTEXT)->definitions,SEM_TYPE_PROTOCOL),ALIVE.id)),"(PROTOCOL_DEFINITION (PROTOCOL_LABEL:ALIVE) (ROLE:SERVER) (ROLE:CLIENT) (GOAL:HANDLER) (alive (EXPECT (ROLE:SERVER) (SOURCE (ROLE:CLIENT)) (PATTERN (SEMTREX_SYMBOL_LITERAL (SEMTREX_SYMBOL:PING))) (ACTION:respond_with_yup)) (EXPECT (ROLE:CLIENT) (SOURCE (ROLE:SERVER)) (PATTERN (SEMTREX_SYMBOL_LITERAL (SEMTREX_SYMBOL:YUP))) (GOAL:HANDLER))))");
}
