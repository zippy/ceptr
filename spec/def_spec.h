...
void testDefSysDefs() {
    ...
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,SYS_CONTEXT)->definitions,SEM_TYPE_PROCESS),IF.id)),"(PROCESS_DEFINITION (PROCESS_NAME:IF) (PROCESS_INTENTION:if) (PROCESS_SIGNATURE (OUTPUT_SIGNATURE (SIGNATURE_LABEL:result) (SIGNATURE_PASSTHRU)) (INPUT_SIGNATURE (SIGNATURE_LABEL:condition) (SIGNATURE_PROCESS:BOOLEAN)) (INPUT_SIGNATURE (SIGNATURE_LABEL:then) (SIGNATURE_ANY)) (INPUT_SIGNATURE (SIGNATURE_LABEL:else) (SIGNATURE_ANY) (SIGNATURE_OPTIONAL))))");

    // an example of a defined protocol (alive)
    spec_is_str_equal(t2s(_t_child(_t_child(__sem_context(G_sem,SYS_CONTEXT)->definitions,SEM_TYPE_PROTOCOL),ALIVE.id)),"(PROTOCOL_DEFINITION (PROTOCOL_LABEL:ALIVE) (ROLE:SERVER) (ROLE:CLIENT) (GOAL:HANDLER) (alive (EXPECT (ROLE:SERVER) (SOURCE (ROLE:CLIENT)) (PATTERN (SEMTREX_SYMBOL_LITERAL (SEMTREX_SYMBOL:PING))) (ACTION:respond with yup)) (EXPECT (ROLE:CLIENT) (SOURCE (ROLE:SERVER)) (PATTERN (SEMTREX_SYMBOL_LITERAL (SEMTREX_SYMBOL:YUP))) (GOAL:HANDLER))))");
}
